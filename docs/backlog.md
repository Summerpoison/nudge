...
- [ ] **Timezone-Einstellung.** Aktuell nutzt die App überall naive lokale Zeit (`datetime.now()` ohne `tzinfo`) — korrekt, solange Server und Nutzerin dieselbe Maschine/Zeitzone teilen, was für den aktuellen Single-User-Local-Betrieb zutrifft. Eine echte Umsetzung würde intern auf UTC-Speicherung umstellen und erst bei der Anzeige in die eingestellte Zeitzone konvertieren (`zoneinfo`, Standardbibliothek seit Python 3.9, keine neue Abhängigkeit) — betrifft Task-Anlage, alle Checkpoint-/Buffer-Berechnungen und jede Datumsanzeige. Bewusst zurückgestellt (mit Nutzerin abgestimmt), da aktuell kein echtes Problem, aber vor einem etwaigen späteren Deployment auf einem Server relevant.
